...
--
-- Create schema hsr4ky_facetedopac
--

CREATE DATABASE IF NOT EXISTS hsr4ky_facetedopac;
USE hsr4ky_facetedopac;

--
-- Temporary table structure for view `book_complete`
--
DROP TABLE IF EXISTS `book_complete`;
DROP VIEW IF EXISTS `book_complete`;
CREATE TABLE `book_complete` (
  `id` int(10) unsigned,
  `title` varchar(256),
  `subject` varchar(128),
  `author` varchar(128),
  `isbn` varchar(25),
  `call_number` varchar(45),
  `location_link` varchar(256),
  `imprint` varchar(256),
...
CREATE TABLE `ref_format` (
  ...
  PRIMARY KEY (`id`),
  KEY `ref_format_id` (`id`),
  KEY `ref_format_value` (`format`)
...
--
-- Definition of view `book_complete`
--

DROP TABLE IF EXISTS `book_complete`;
DROP VIEW IF EXISTS `book_complete`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `book_complete` AS select `book`.`id` AS `id`,`book`.`title` AS `title`,`book_subject`.`subject` AS `subject`,`book_author`.`author` AS `author`,`book_isbn`.`isbn` AS `isbn`,`book`.`call_number` AS `call_number`,`book`.`location_link` AS `location_link`,`book`.`imprint` AS `imprint`,`book`.`year` AS `year`,`book`.`publisher` AS `publisher`,`book`.`description` AS `description`,`ref_format`.`format` AS `format`,`ref_language`.`language` AS `language`,`ref_location`.`location` AS `location` from ((((((`book` join `book_author`) join `book_isbn`) join `book_subject`) join `ref_format`) join `ref_language`) join `ref_location`) where ((`book`.`format` = `ref_format`.`id`) and (`book`.`language` = `ref_language`.`id`) and (`book`.`location` = `ref_location`.`id`) and (`book_author`.`book_id` = `book`.`id`) and (`book_isbn`.`book_id` = `book`.`id`) and (`book_subject`.`book_id` = `book`.`id`)) order by `book`.`id`;
